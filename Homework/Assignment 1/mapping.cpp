...
dataType::~dataType()
{
  ...
  delete [] desc;  //Deallocates the memory for the descripion
}
...
int mappingType::insertItem(const dataType & newData)
{
  ...
  //Copies Description
  len = strlen(newData.desc);
  current->data[arrayIndex].desc = new char [len + 1];
  strcpy(current->data[arrayIndex].desc, newData.desc);
...
int mappingType::search(int id, dataType & foundData)
{
  ...
  {
    //Copies Name
    len = strlen(current->data[arrayIndex].name);
    foundData.name = new char [len + 1];
    strcpy(foundData.name, current->data[arrayIndex].name);
    //Copies Type
    len = strlen(current->data[arrayIndex].type);
    foundData.type = new char [len + 1];
    strcpy(foundData.type, current->data[arrayIndex].type);
    //Copies Memory location
    foundData.memoryLocation = current->data[arrayIndex].memoryLocation;
    //Copies Description
    len = strlen(current->data[arrayIndex].desc);
    foundData.desc = new char [len + 1];
    strcpy(foundData.desc, current->data[arrayIndex].desc);
...
//   cout << "--Location:" << traversal << "-" << arrayIndex << " " << endl;  //Very useful for debugging
    ...
    if(current->data[arrayIndex].valid)  //If the current array is not "deleted"
    {
      ...
      cout << "--Description: " << current->data[arrayIndex].desc << endl;       //Outputs the description
