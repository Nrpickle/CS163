...
struct dataType 
{
  ...
  char * type; //Dynamically allocated array to hold the type of the item
  int memoryLocation; //Stores the made up memory location
  char * desc; //Dynamically allocated array to hold the description of the item
};
...
class mappingType 
{
  public:
    ...
    int insertItem(const dataType & newData); //Inserts the passed item
