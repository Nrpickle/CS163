...
bool manager(menuType & menu, stackType & stack);   //Manages the main repeating functionality of the program
...
int main()
{
  ...
  while(manager(menu,stack));

  return 0;
}


bool manager(menuType & menu, stackType & stack)
{
  menu.disp();
  menu.choose();
  menu.process(stack);
