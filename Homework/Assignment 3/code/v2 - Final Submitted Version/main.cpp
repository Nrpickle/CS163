...
bool manager(menuType & menu, tableType & table);   //Manages the main 
...
int main()
{
  ...
  tableType table;  //Creates an instance of a hash table to store information
  
  while(manager(menu, table)); //The main loop of the program
...
bool manager(menuType & menu, tableType & table)
{
  menu.disp();
  menu.choose();
  menu.process(table);
