...
//Prototypes
bool manager(menuType & menu, graphType & graph);    //Manages the program and the menu class. It is located in this file
...
int main()
{
  ...
  graphType graph;  //Declares the Client Program class

  while(manager(menu,graph));   //The main executing loop

  return 0;
}

//Manages the Client Application, and the looping
bool manager(menuType & menu, graphType & graph)
{
  menu.disp();          //Displays the menu
  menu.choose();        //Prompts the user to choose a selection
  menu.process(graph);    //Processes the selection
